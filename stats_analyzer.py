...
stat_map["ramulator.L3_cache_total_miss"] = "LLC_cache_total_miss"
stat_map["ramulator.L3_cache_total_access"] = "LLC_cache_total_access"
...
stat_map["ramulator.total_energy_"] = "total_energy_per_chan"
stat_map["ramulator.average_power_"] = "average_power_per_chan"
stat_map["ramulator.act_energy_"] = "act_energy_per_chan"
stat_map["ramulator.pre_energy_"] = "pre_energy_per_chan"
stat_map["ramulator.read_energy_"] = "read_energy_per_chan"
stat_map["ramulator.write_energy_"] = "write_energy_per_chan"
stat_map["ramulator.act_stdby_energy_"] = "act_stdby_energy_per_chan"
stat_map["ramulator.pre_stdby_energy_"] = "pre_stdby_energy_per_chan"
stat_map["ramulator.ref_energy_"] = "ref_energy_per_chan"
stat_map["ramulator.io_term_energy_"] = "io_term_energy_per_chan"
...
multidim_stats.append("ramulator.total_energy_")
multidim_stats.append("ramulator.average_power_")
multidim_stats.append("ramulator.act_energy_")
multidim_stats.append("ramulator.pre_energy_")
multidim_stats.append("ramulator.read_energy_")
multidim_stats.append("ramulator.write_energy_")
multidim_stats.append("ramulator.act_stdby_energy_")
multidim_stats.append("ramulator.pre_stdby_energy_")
multidim_stats.append("ramulator.ref_energy_")
multidim_stats.append("ramulator.io_term_energy_")
#
...
class Stats(object):
  ...
  def init_from_file(self, f, fname):
  ...
  # preprocess second-level statistics
    ...
    try:
      # scale for total energy/energy breakdown/average power (sum up for per channel)
      if "DDR3-2133L" in fname or "DDR4-2400R" in fname:
        scale = 4
      elif "LPDDR3-2133" in fname:
        scale = 2
      else:
        scale = 1
      mem_stats["total_energy"] = sum([s[1] for s in mem_stats["total_energy_per_chan"][0]]) * scale
      mem_stats["average_power"] = sum([s[1] for s in mem_stats["average_power_per_chan"][0]]) * scale
      mem_stats["act_energy"] = sum([s[1] for s in mem_stats["act_energy_per_chan"][0]]) * scale
      mem_stats["pre_energy"] = sum([s[1] for s in mem_stats["pre_energy_per_chan"][0]]) * scale
      mem_stats["read_energy"] = sum([s[1] for s in mem_stats["read_energy_per_chan"][0]]) * scale
      mem_stats["write_energy"] = sum([s[1] for s in mem_stats["write_energy_per_chan"][0]]) * scale
      mem_stats["act_stdby_energy"] = sum([s[1] for s in mem_stats["act_stdby_energy_per_chan"][0]]) * scale
      mem_stats["pre_stdby_energy"] = sum([s[1] for s in mem_stats["pre_stdby_energy_per_chan"][0]]) * scale
      mem_stats["ref_energy"] = sum([s[1] for s in mem_stats["ref_energy_per_chan"][0]]) * scale
      mem_stats["io_term_energy"] = sum([s[1] for s in mem_stats["io_term_energy_per_chan"][0]]) * scale
      mem_stats["act_power"] = mem_stats["average_power"] * mem_stats["act_energy"] / mem_stats["total_energy"]
      mem_stats["pre_power"] = mem_stats["average_power"] * mem_stats["pre_energy"] / mem_stats["total_energy"]
      mem_stats["read_power"] = mem_stats["average_power"] * mem_stats["read_energy"] / mem_stats["total_energy"]
      mem_stats["write_power"] = mem_stats["average_power"] * mem_stats["write_energy"] / mem_stats["total_energy"]
      mem_stats["act_stdby_power"] = mem_stats["average_power"] * mem_stats["act_stdby_energy"] / mem_stats["total_energy"]
      mem_stats["pre_stdby_power"] = mem_stats["average_power"] * mem_stats["pre_stdby_energy"] / mem_stats["total_energy"]
      mem_stats["ref_power"] = mem_stats["average_power"] * mem_stats["ref_energy"] / mem_stats["total_energy"]
      mem_stats["io_term_power"] = mem_stats["average_power"] * mem_stats["io_term_energy"] / mem_stats["total_energy"]
    except KeyError as detail:
      print "Ignore KeyError:", detail
      pass

    try:
      if len(mem_stats["ipc"]) == 0: # SoC, bandwidth per watt
        mem_stats["perf_per_mw"] = mem_stats["bandwidth"] / mem_stats["average_power"]
      elif len(mem_stats["ipc"]) == 1: # Single-threaded workload, ipc per watt
        mem_stats["perf_per_mw"] = mem_stats["ipc"][0] / mem_stats["average_power"]
      else: # multiprogrammed workload, weighted-speedup per watt
        mem_stats["perf_per_mw"] = mem_stats["weighted_speedup"] / mem_stats["average_power"]
    except KeyError as detail:
      print "Ignore KeyError: ", detail
      pass

    try:
      if not "LLC_cache_total_access" in mem_stats or mem_stats["LLC_cache_total_access"] == 0:
        mem_stats["LLC_miss_rate"] = 0
      else:
        mem_stats["LLC_miss_rate"] = mem_stats["LLC_cache_total_miss"] / mem_stats["LLC_cache_total_access"]
    except KeyError as detail:
      print "Ignore KeyError: ", detila
      pass
    ...
    if not ref_fname_list is None:
      for ref_fname in ref_fname_list:
        if os.path.isfile(ref_fname):
          ...
          ref_mem_stats, ref_sys_stats = self.init_from_file(ref_fdesc, ref_fname)
          self.ref_mem_stats.append(ref_mem_stats)
          self.ref_sys_stats.append(ref_sys_stats)

    self.mem_stats, self.sys_stats = self.init_from_file(target_fdesc, target_fname)
